...
public class CsvEvaluator {
        ...
        for (CSVRecord record : parser) {
            ...
            for (String cellValue : record) {
                if (!cellValue.isEmpty() && cellValue.charAt(0) == '=') {
                    try {
                        Expression expression = RpnParser.parse(cellValue.substring(1), defs);
                        double expressionValue = expression.eval(vars);
                        vars.set(colToLetters(currentColumn) + currentRow, expressionValue);
                    ...
                    try {
                        vars.set(colToLetters(currentColumn) + currentRow, Double.parseDouble(cellValue));
    ...
    public static String colToLetters(int n) {
        ...
        //Recursion
        return colToLetters(q) + (char) ('A' + r - 1);
    }
